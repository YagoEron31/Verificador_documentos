...
# Mantenha esta linha para desenvolvimento local.
# No Render, as variáveis de ambiente devem ser configuradas diretamente no dashboard.
load_dotenv()
    ...
    try:
        with conn.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                VALUES (%s, %s, %s, %s, %s, %s)
            """, (hash_conteudo, texto_extraido, analise['status'], json.dumps(analise['erros']), caminho_storage, datetime.now())) # <-- AJUSTE AQUI: json.dumps para lista de erros
...
def upload_file():
    ...
    try:
        ...
        # Imprime o erro completo para facilitar o debug nos logs do Render
...
# Removida a execução local do app.run() para compatibilidade com o Render.
# O Render (via Gunicorn) se encarregará de iniciar a aplicação.
